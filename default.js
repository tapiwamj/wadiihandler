          ...
          con.query("SELECT * FROM questions", function (err, result, fields) {
            ...
            if (questionsCount > 0) {
              if (questionsCount == 1) {
                ...
                questionIndex = Math.floor(Math.random() * (questionsCount - 0)) + 0;
              }
              message1 = {
                instruction: 'question',
                question: result[questionIndex].question
              }
              message2 = {
                instruction: 'question',
                question: result[questionIndex].question
              ...
              if (intersection.length > 0) {
                ...
                if (sessionmode == 'question') {
                  ...
                  con.query("SELECT * FROM questions", function (err, result, fields) {
                    ...
                    questionsCount = result.length;
                    if (questionsCount > 0) {
                      var questionIndex = Math.floor(Math.random() * (questionsCount - 1)) + 1;
                      message1.question = result[questionIndex].question;
                      message2.question = result[questionIndex].question;
                    }
                  });
                }
                clientsLibrary[searcherIndex].socket.send(JSON.stringify(message1));
